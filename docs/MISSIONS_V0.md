...
Runs poll through the Mission Pod; this is persisted-state polling, not token streaming. A checkpoint agent waits for an owner or admin decision before the certified executor is invoked. An expired lease after the executor start marker is intentionally held for human retry, because its side effects are uncertain. Only queued or approval-waiting runs can be cancelled.
