...
export function MissionPod({
  ...
  return <div className="mission-panel-scrim" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    ...
    {agentOpen ? <div className="mission-agent-modal-scrim"><form className="mission-agent-modal" onSubmit={submitAgent}><header><div><span>CREATE AGENT</span><h2>Add a specialist</h2><p>Define the role. Missions manages runtime and model routing.</p></div><button type="button" aria-label="Close agent form" onClick={() => setAgentOpen(false)}><X size={18} /></button></header><div className="mission-agent-fields"><label><span>Name</span><input autoFocus required placeholder="e.g. Reconciliation analyst" value={agent.name} onChange={(event) => setAgent({ ...agent, name: event.target.value })} /></label><label><span>Role</span><input required placeholder="e.g. Finance operations" value={agent.role} onChange={(event) => setAgent({ ...agent, role: event.target.value })} /></label><label className="wide"><span>Description</span><small>What outcome does this agent own?</small><textarea required placeholder="e.g. Reconcile invoices, surface exceptions, and prepare evidence for human review." value={agent.mandate} onChange={(event) => setAgent({ ...agent, mandate: event.target.value })} /></label><label className="wide"><span>Responsibilities</span><input placeholder="e.g. Match records, investigate exceptions, prepare a review pack" value={agent.responsibilities} onChange={(event) => setAgent({ ...agent, responsibilities: event.target.value })} /></label><label><span>Data access</span><input placeholder="e.g. invoices/, purchase-orders/" value={agent.dataScope} onChange={(event) => setAgent({ ...agent, dataScope: event.target.value })} /></label><label><span>Capabilities</span><input placeholder="e.g. document.read, artifact.write" value={agent.tools} onChange={(event) => setAgent({ ...agent, tools: event.target.value })} /></label><label className="wide"><span>Autonomy</span><select value={agent.autonomy} onChange={(event) => setAgent({ ...agent, autonomy: event.target.value as MissionAgentInput["autonomy"] })}><option value="assist">Assist — draft for a human</option><option value="execute_safely">Execute safely — act within scope</option><option value="operate_with_checkpoints">Checkpoints — ask before consequential work</option></select></label><label><span>Tool limit <em>Optional</em></span><input type="number" min="1" max="100" placeholder="Use Mission default" value={agent.maxSteps} onChange={(event) => setAgent({ ...agent, maxSteps: event.target.value })} /></label><label><span>Turn limit <em>Optional</em></span><input type="number" min="1" max="600" placeholder="Use Mission default" value={agent.wallTimeout} onChange={(event) => setAgent({ ...agent, wallTimeout: event.target.value })} /></label></div><footer><button type="button" className="secondary" onClick={() => setAgentOpen(false)}>Cancel</button><button type="submit"><Plus size={14} /> Create agent</button></footer></form></div> : null}
  </div>;
}
