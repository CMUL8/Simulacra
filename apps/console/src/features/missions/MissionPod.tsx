...
export function MissionPod({ projectId, projectTitle, projectPrompt = "", artifactKind = "report", onClose }: { projectId: string; projectTitle: string; projectPrompt?: string; artifactKind?: string; onClose: () => void }) {
  ...
  return <section className="mission-pod" aria-label="Mission Pod">
    <header className="mission-header"><div><span className="mission-kicker">MISSION</span><h2>{projectTitle}</h2></div><div><button onClick={() => void queueRun()} disabled={!canQueue} title={!canQueue ? "Complete the readiness checks before running" : "Queue a governed agent run"}>{canQueue ? "Run Mission" : "Not ready"}</button><button className="mission-quiet" onClick={onClose}>Back to chat</button></div></header>
      ...
      {tab === "Overview" && <div className="mission-overview">
        <section className="mission-readiness" aria-label="Mission readiness">
          ...
          <div className="mission-checks">
            ...
            <span className={crewReady ? "ready" : ""}>{crewReady ? "✓" : "3"} {crewReady ? `${data.agents.length} agent${data.agents.length === 1 ? "" : "s"} configured` : "Add an agent"}</span>
