...
export function MissionPod({
  ...
  return <div className="mission-panel-scrim" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}>
    ...
    {agentOpen ? <div className="mission-agent-modal-scrim"><form className="mission-agent-modal" onSubmit={submitAgent}><header><div><span>CREATE AGENT</span><h2>Add a specialist</h2><p>Define who they are and what they own. Missions handles execution.</p></div><button type="button" aria-label="Close agent form" onClick={() => setAgentOpen(false)}><X size={18} /></button></header><div className="mission-agent-fields"><div className="mission-agent-presets wide"><span>START WITH A ROLE</span><button type="button" onClick={() => setAgent({ ...emptyAgent, name: "Reconciliation analyst", role: "Finance operations", mandate: "Reconcile records, surface exceptions, and prepare exact evidence for human review.", responsibilities: "Match records, investigate exceptions, prepare review pack", dataScope: "invoices/, purchase-orders/", tools: "document.read, artifact.write", autonomy: "operate_with_checkpoints" })}>Reconciliation</button><button type="button" onClick={() => setAgent({ ...emptyAgent, name: "Research analyst", role: "Research and synthesis", mandate: "Ground findings in Mission sources and produce a reviewable, cited brief.", responsibilities: "Read sources, compare evidence, draft report", dataScope: "sources/", tools: "document.read, artifact.write" })}>Research</button><button type="button" onClick={() => setAgent({ ...emptyAgent, name: "Product builder", role: "Application delivery", mandate: "Turn approved requirements and source data into a small working application for verification.", responsibilities: "Implement changes, test output, prepare evidence", dataScope: "sources/, app/", tools: "document.read, code.write", autonomy: "operate_with_checkpoints" })}>Builder</button></div><label><span>Name</span><input autoFocus required placeholder="e.g. Reconciliation analyst" value={agent.name} onChange={(event) => setAgent({ ...agent, name: event.target.value })} /></label><label><span>Role</span><input required placeholder="e.g. Finance operations" value={agent.role} onChange={(event) => setAgent({ ...agent, role: event.target.value })} /></label><label className="wide"><span>Description</span><small>What outcome does this agent own?</small><textarea required placeholder="Describe the agent's purpose and the evidence it should return." value={agent.mandate} onChange={(event) => setAgent({ ...agent, mandate: event.target.value })} /></label><label className="wide"><span>Autonomy</span><select value={agent.autonomy} onChange={(event) => setAgent({ ...agent, autonomy: event.target.value as MissionAgentInput["autonomy"] })}><option value="assist">Assist — draft for a human</option><option value="execute_safely">Execute safely — act within approved scope</option><option value="operate_with_checkpoints">Checkpoints — ask before consequential work</option></select></label><details className="mission-agent-advanced wide"><summary>Advanced scope and limits</summary><label><span>Responsibilities</span><input placeholder="Comma-separated responsibilities" value={agent.responsibilities} onChange={(event) => setAgent({ ...agent, responsibilities: event.target.value })} /></label><div><label><span>Data access</span><input placeholder="e.g. invoices/, purchase-orders/" value={agent.dataScope} onChange={(event) => setAgent({ ...agent, dataScope: event.target.value })} /></label><label><span>Capabilities</span><input placeholder="e.g. document.read, artifact.write" value={agent.tools} onChange={(event) => setAgent({ ...agent, tools: event.target.value })} /></label></div><div><label><span>Tool actions</span><input type="number" min="1" max="100" placeholder="Mission default" value={agent.maxSteps} onChange={(event) => setAgent({ ...agent, maxSteps: event.target.value })} /></label><label><span>Turn seconds</span><input type="number" min="1" max="600" placeholder="Mission default" value={agent.wallTimeout} onChange={(event) => setAgent({ ...agent, wallTimeout: event.target.value })} /></label></div></details></div><footer><button type="button" className="secondary" onClick={() => setAgentOpen(false)}>Cancel</button><button type="submit"><Plus size={14} /> Create agent</button></footer></form></div> : null}
  </div>;
}
