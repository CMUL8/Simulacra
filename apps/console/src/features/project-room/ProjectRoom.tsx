...
export function ProjectRoom({ room, state = "ready", permissions = DEFAULT_PERMISSIONS, adapter, onRetryLoad, onOpenGraph, onOpenActivity, onInvite, actionError }: ProjectRoomProps) {
  ...
  return <main className="cm-room" aria-labelledby="cm-room-title"><header className="cm-room__header"><div><span className="cm-room__eyebrow">Project Room</span><h1 id="cm-room-title">{room.name}</h1>{room.context.objective ? <p>{room.context.objective}</p> : null}</div><div className="cm-room__header-actions">{onOpenActivity ? <button type="button" onClick={onOpenActivity}><Activity size={14} /> Activity</button> : null}{onOpenGraph ? <button type="button" onClick={onOpenGraph}><GitBranch size={14} /> Review graph</button> : null}</div></header>
    ...
    <div className="cm-room__columns"><TaskBoard tasks={room.tasks} members={room.members} canManage={permissions.manageTasks} canReview={permissions.reviewTasks} onClaim={adapter?.claimTask ? (id, revision) => void adapter.claimTask?.(id, revision) : undefined} onTransition={adapter?.transitionTask ? (id, next, revision) => void adapter.transitionTask?.(id, next, revision) : undefined} onReview={adapter?.submitTaskReview ? (id, decision, note, revision) => void adapter.submitTaskReview?.(id, decision, note, revision) : undefined} /><RoomRoster members={room.members} canInvite={permissions.invite} onInvite={onInvite} onInviteMember={adapter?.addMember ? (memberId, role) => void adapter.addMember?.(memberId, role, room.revision) : undefined} /></div>
...
function RoomRoster({ members, canInvite, onInvite, onInviteMember }: { members: RoomMember[]; canInvite: boolean; onInvite?: () => void; onInviteMember?: (memberId: string, role: "owner" | "admin" | "member" | "viewer" | "reviewer" | "approver") => void }) {
  ...
  return <section className="cm-room-roster" aria-labelledby="cm-room-roster-title"><header><div><span>Durable membership</span><h2 id="cm-room-roster-title">Team</h2></div>{canInvite && (onInviteMember || onInvite) ? <button type="button" onClick={() => onInviteMember ? setInviteOpen((value) => !value) : onInvite?.()}>Invite</button> : null}</header>{inviteOpen ? <form className="cm-room-roster__invite" onSubmit={(event) => { event.preventDefault(); invite(); }}><label>Member ID<input value={memberId} onChange={(event) => setMemberId(event.target.value)} placeholder="Authenticated member ID" required /></label><label>Role<select value={role} onChange={(event) => setRole(event.target.value as typeof role)}>{["member", "viewer", "reviewer", "approver", "admin", "owner"].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><button type="submit">Add member</button></form> : null}{members.length ? <ul>{members.map((member) => <li key={member.id}>{member.kind === "agent" ? <Bot size={14} /> : <CircleUserRound size={14} />}<span><strong>{member.name}</strong><small>{member.role}</small></span>{member.presence ? <em>{member.presence}</em> : <em>Presence not reported</em>}</li>)}</ul> : <p>No members are recorded for this room.</p>}</section>;
}
