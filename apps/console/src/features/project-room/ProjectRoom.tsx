...
export function ProjectRoom({ room, state = "ready", permissions = DEFAULT_PERMISSIONS, adapter, missionAssignments = [], missionApprovals = [], missionDeliverables = [], onRetryLoad, onOpenChat, onInvite, actionError }: ProjectRoomProps) {
  ...
  const needsAttention = room.tasks.filter((task) => task.durableState === "blocked" || !task.ownerId);
  const working = room.tasks.filter((task) => ["working", "ready", "proposed"].includes(task.durableState));
  const readyForReview = room.tasks.filter((task) => task.durableState === "in_review");
  const done = room.tasks.filter((task) => task.durableState === "done");
	const pendingMissionApprovals = missionApprovals.filter((item) => item.status === "awaiting_approval");
	const closedMissionApprovals = missionApprovals.filter((item) => item.status === "closed");
	const progressingRunStatuses = ["running", "queued", "preparing", "verifying"];
	const successfulRunStatuses = ["succeeded"];
	const stoppedRuns = missionAssignments.filter((item) => ![...progressingRunStatuses, "awaiting_approval", ...successfulRunStatuses].includes(item.status));
	const stoppedTasks = room.tasks.filter((task) => ["cancelled", "failed"].includes(task.durableState));
	const activeAssignments = missionAssignments.filter((item) => [...progressingRunStatuses, "awaiting_approval"].includes(item.status));
  ...
  return <main className="cm-room cm-room--tasks" aria-labelledby="cm-room-title"><header className="cm-room__header"><div><span className="cm-room__eyebrow">MISSION WORK</span><h1 id="cm-room-title">Work</h1><p>Assignments, progress, and decisions—one shared queue for humans and agents.</p></div><div className="cm-room__header-actions">{onOpenChat ? <button type="button" onClick={onOpenChat}>Assign work <ArrowRight size={14} /></button> : null}</div></header>
    ...
    {planNeedsReview ? <section className="cm-room-plan" aria-label="Mission plan review"><header><span><ShieldCheck size={14} /> BEFORE THE FIRST RUN</span><h2>Approve how the crew will work</h2><p>Confirm the responsibilities and access the Mission prepared from your outcome.</p></header>{room.graph?.summaries.length ? <ul>{room.graph.summaries.slice(0, 4).map((item) => <li key={item.id}><strong>{item.name}</strong><small>{item.detail}</small></li>)}</ul> : null}<div>{permissions.reviewGraph && adapter?.approveGraph ? <button type="button" onClick={() => void adapter.approveGraph?.(room.graph!.revision)}>Approve and continue</button> : <span>Waiting for a Mission owner to approve.</span>}</div></section> : null}
    <section className="cm-task-overview" aria-label="Work status"><article className={needsAttention.length || pendingMissionApprovals.length || activeAssignments.some((item) => item.status === "awaiting_approval") ? "attention" : ""}><span>Needs you</span><strong>{needsAttention.length + pendingMissionApprovals.length + activeAssignments.filter((item) => item.status === "awaiting_approval").length}</strong><small>Review, unblock, or assign</small></article><article><span>In progress</span><strong>{working.length + activeAssignments.filter((item) => progressingRunStatuses.includes(item.status)).length}</strong><small>Humans and agents moving work forward</small></article><article><span>Ready for review</span><strong>{readyForReview.length + missionDeliverables.filter((item) => item.status === "ready_for_review").length}</strong><small>Evidence waiting for a human decision</small></article><article><span>Done</span><strong>{done.length + missionAssignments.filter((item) => successfulRunStatuses.includes(item.status)).length + missionDeliverables.filter((item) => item.status === "done").length + missionApprovals.filter((item) => item.status === "done").length}</strong><small>Finished work and evidence</small></article><article><span>Stopped / Closed</span><strong>{stoppedTasks.length + stoppedRuns.length + closedMissionApprovals.length}</strong><small>Work that did not finish successfully</small></article></section>
    <div className="cm-room__columns"><TaskBoard tasks={room.tasks} assignments={missionAssignments} approvals={missionApprovals} deliverables={missionDeliverables} members={room.members} canManage={permissions.manageTasks} canReview={permissions.reviewTasks} canDecideMission={permissions.reviewGraph} onOpenChat={onOpenChat} onClaim={adapter?.claimTask ? (id, revision) => void adapter.claimTask?.(id, revision) : undefined} onTransition={adapter?.transitionTask ? (id, next, revision) => void adapter.transitionTask?.(id, next, revision) : undefined} onReview={adapter?.submitTaskReview ? (id, decision, note, revision) => void adapter.submitTaskReview?.(id, decision, note, revision) : undefined} onDecideApproval={(id, decision, revision, runRevision) => void adapter?.decideMissionApproval?.(id, decision, revision, runRevision)} onVerifyDeliverable={(id, version) => void adapter?.verifyMissionDeliverable?.(id, version)} /></div>
