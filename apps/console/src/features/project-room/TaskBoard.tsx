...
export function TaskBoard({ tasks, assignments = [], members, canManage, canReview, onClaim, onTransition, onReview, onOpenChat }: {
  ...
  const items = useMemo<WorkItem[]>(() => [
    ...
    ...tasks.map((task): WorkItem => { const owner = members.find((member) => member.id === task.ownerId); return { kind: "room", id: `task:${task.id}`, title: task.title, status: task.durableState, owner: owner?.name ?? "Unassigned", detail: task.detail || "Durable Mission task", task }; }),
  ...
  const groups = [
    ...
    { id: "working", title: "Working now", detail: "An agent or teammate is active", items: items.filter((item) => bucket(item) === "working") },
